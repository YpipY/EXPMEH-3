...
opti <- function(filename) {
  ...
  ans = optimizeParam(data$down.HR1, data$down.HR2, par, min.rec = 3.5, max.rec = 4.5)
...
for (d in files_prep) {
  p = try(expr= opti(d))
  opti_data = try( expr = rbind(opti_data, p))
...
recur <- function(filename) {
  ...
  Results=crqa(data$down.HR1, data$down.HR2, delay=[[3]], embed=ans[[2]], radius=ans[[1]], normalize=0, rescale=0, mindiagline = 2, minvertline = 2)
