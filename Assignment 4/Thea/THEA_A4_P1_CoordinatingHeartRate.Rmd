...
setwd("~/EXPMETH3/Assignment 4/Thea")
...
theaspath_HR = "/Users/thearolskovsloth/Documents/3. SEMESTER/Experimental Methods 3/Portfolios/Portfolio 7/plotz_HR/"
files <- list.files(path = path)

for (d in files) {
  p = loop_plot_HR(d)
  ggsave(p, file = paste(theaspath_HR, d, "_HR", ".png", sep = ""))
...
theaspath_Resp = "/Users/thearolskovsloth/Documents/3. SEMESTER/Experimental Methods 3/Portfolios/Portfolio 7/plotz_Resp/"
...
for (d in files) {
  p = loop_plot_Resp(d)
  ggsave(p, file = paste(theaspath_Resp, d, "_resp", ".png", sep = ""))
...
for (d in files) {
  p = prep(d)
  write.csv(p, file = paste(prep_path,"prep_",d, sep = ""))
...
```{r}
#Visually inspect the preprocessed data 


path_prep = "/Users/thearolskovsloth/EXPMETH3/Assignment 4/Thea/prep/"

loop_plot_HR_prep <- function(filename) {
  data <- read.csv(file = paste("/Users/thearolskovsloth/EXPMETH3/Assignment 4/Thea/prep/ ",filename, sep = ""), sep = ",", header = TRUE) 
  plot = ggplot(data = data, aes(x = time)) + geom_line(aes(y = down.HR1, colour = "down.HR1")) + geom_line(aes(y = down.HR2, colour = "down.HR2")) + labs(title= filename, y = "Heart Rate")
  
  return(plot)
  
}

loop_plot_HR_prep("prep_Study1_G1_T1_Synchronous.csv")

data2 <- read.delim(file = paste("/Users/thearolskovsloth/EXPMETH3/Assignment 4/Thea/prep/ ",filename, sep = ""), sep = ",", header = TRUE) 
 
 #HR

theaspath_prep_HR = "/Users/thearolskovsloth/EXPMETH3/Assignment 4/Thea/plotz_prep_HR/"
files_prep <- list.files(path = path_prep)

for (d in files_prep) {
  p = loop_plot_HR(d)
  ggsave(p, file = paste(theaspath_prep_HR, d, "_HR", ".png", sep = ""))
...
theaspath_prep_Resp = "/Users/thearolskovsloth/EXPMETH3/Assignment 4/Thea/prep/plotz_prep_Resp/"

loop_plot_Resp_prep <- function(filename) {
  data <- read.csv(file = paste(path_prep, filename, sep = ""), sep = ",", header = TRUE) 
  plot = ggplot(data = data, aes(x = time)) + geom_line(aes(y = down.Resp1, colour = "down.Resp1")) + geom_line(aes(y = down.Resp2, colour = "down.Resp2")) + labs(title= filename, y = "Respiration")
...
for (d in files_prep) {
  p = loop_plot_Resp(d)
  ggsave(p, file = paste(theaspath_prep_Resp, d, "_resp", ".png", sep = ""))
...
#crqa(d$down.HR1, d$down.HR2, delay = 1, embed = 2, normalize = 0, rescale = 0, radius = 2, mindiagline = 2, minvertline = 2, tw = 1, whiteline = FALSE, recpt = F, side = 3, checkl = 1)
d <- read.csv("prep_Study1_G2_T1_TurnTaking.csv")
...
setwd("/Users/thearolskovsloth/Documents/3. SEMESTER/Experimental Methods 3/Portfolios/Portfolio 7/prep/")
...
par = list(lgM = 50, steps = seq(1, 6, 1), radiusspan = 200, radiussample = 40, normalize = 0, rescale = 0, mindiagline = 2, minvertline = 2, tw = 0, whiteline = FALSE, recpt = FALSE, fnnpercent = 10, typeami = "mindip")

#OPTIMZING
filename = "prep_Study1_G3_T3_Synchronous.csv"

crqa_path = "/Users/thearolskovsloth/Documents/3. SEMESTER/Experimental Methods 3/Portfolios/Portfolio 7/prep/"

opti <- function(filename) {
  #read data
  data <- read.csv(file = paste(crqa_path, filename, sep = ""), sep = ",", header = TRUE)
  
  # parse filename
  meta = str_extract_all(filename, "\\d+")
  cond = substr(filename, 19, 24)
  study = as.numeric(meta[[1]][1])
  pair = as.numeric(meta[[1]][2])
  trial = as.numeric(meta [[1]][3])
  ...
  ans = rep(NA, 3)
  try(expr = (ans = optimizeParam(data$down.HR1, data$down.HR2, par, min.rec = 2, max.rec = 5)))
  ...
  output = data.frame(pair, study, cond, trial, delay, embed, radius)
...
#test
c <- opti("prep_Study1_G2_T1_TurnTaking.csv")
c

#Iterate thorugh all
opti_data <- data.frame()
#colnames(opti_data) <- c("pair", "study", "cond", "trial", "delay", "embed", "radius")


for (d in files_prep) {
  p= opti(d)
  print(d)
  opti_data = rbind(opti_data, p)
...
recur <- function(filename) {
  ...
  Results=crqa(data$down.HR1, data$down.HR2, delay= 22, embed= 11, radius= 1.65, normalize=0, rescale=0, mindiagline = 2, minvertline = 2)
...
# filename = "prep_Study1_G3_T3_Synchronous.csv"
#data <- read.csv(file = paste(crqa_path, filename, sep = ""), sep = ",", header = TRUE)
# datag <- read.csv(filename)

c <- recur("prep_Study1_G3_T3_Synchronous.csv")
c


recur_data <- data.frame()

for (d in files_prep) {
  try(expr= ( p= recur(d)))
  print(d)
  try(expr= (recur_data = rbind(recur_data, p)))
...
recur_shuffle <- function(filename) {
  ...
  Results=crqa(sample(data$down.HR1), sample(data$down.HR2), delay= 22, embed= 11, radius= 1.65, normalize=0, rescale=0, mindiagline = 2, minvertline = 2)
...
recur_data_shuffle <- data.frame()

for (d in files_prep) {
  try(expr= ( p= recur_shuffle(d)))
  print(d)
  try(expr= (recur_data_shuffle = rbind(recur_data_shuffle, p)))
...
How do we compare with these useless numbers?
...
for (d in files_prep) {
  data <- read.csv(file = paste(crqa_path, d, sep = ""), sep = ",", header = TRUE)
...
recur_data_sur <- data.frame()

for (d in files_prep) {
  try(expr= ( p= recur_sur(d)))
  print(d)
  try(expr= (recur_data_sur = rbind(recur_data_sur, p)))
